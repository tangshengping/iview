...
import iView from 'iview';//iview这个vue库
import VueRouter from 'vue-router';//vue路由
import {routers, otherRouter, appRouter} from './router';//vue自定义路由
import Vuex from 'vuex';//vuex状态管理模式
import Util from './libs/util';
import App from './app.vue';
import Cookies from 'js-cookie';
import 'iview/dist/styles/iview.css';

import VueI18n from 'vue-i18n';//vue.js国际化
...
//目前想实现的功能就是每个路由进入前进行判断，满足条件才进行跳转，否则跳转到特定的页面
...
new Vue({
    ...
    //实例创建完成之后的钩子函数，将菜单列表信息存放在store中
    created () {
        let tagsList = [];
        appRouter.map((item) => {
            if (item.children.length <= 1) {
